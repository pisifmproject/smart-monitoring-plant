...
router.get("/all", async (req, res) => {
  try {
    ...
    const { getCurrentShift } = await import("./lvmdp_1.dailyReport.services");
    ...
    if (needsCurrentShift) {
      try {
        const { saveCurrentShiftReport } = await import(
          "./lvmdp_1.dailyReport.services"
        );
        await saveCurrentShiftReport();
        // Refresh reports after save
        reports = await fetchAllDailyReports();
      } catch (err) {
    ...
    const data = reports.map((r) => ({
      ...
      // Stored totalKwh uses correct formula: real_power × (3/3600)
      shift1TotalKwh:
        r.shift1TotalKwh ?? (r.shift1AvgKwh || 0) * (r.shift1Count || 1),
      shift1CosPhi: r.shift1AvgCosPhi || 0,
      shift2TotalKwh:
        r.shift2TotalKwh ?? (r.shift2AvgKwh || 0) * (r.shift2Count || 1),
      shift2CosPhi: r.shift2AvgCosPhi || 0,
      shift3TotalKwh:
        r.shift3TotalKwh ?? (r.shift3AvgKwh || 0) * (r.shift3Count || 1),
