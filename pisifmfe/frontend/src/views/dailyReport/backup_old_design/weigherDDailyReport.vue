...
const lineId = "LINE_D_WEIGHER";
const lineName = "Line D Weigher";
...
const fetchDailyReport = async () => {
  ...
  try {
    const response = await fetch(
      `http://localhost:2000/api/daily-report/weigher/${lineId}?date=${selectedDate.value}`
    );
    ...
    <div class="daily-report-container">
      ...
      <div class="header-section">
        <div>
          ...
          <p class="page-subtitle">Packing Weigher Report</p>
        </div>
      ...
      <div v-else-if="reportData" class="report-content">
        ...
        <div class="summary-section">
          <div class="summary-card">
            <div class="summary-label">Total Target Packs</div>
            <div class="summary-value">
              {{ reportData.summary.totalTargetPacks }}
            </div>
            <div class="summary-unit">packs</div>
          </div>
          <div class="summary-card">
            <div class="summary-label">Total Actual Packs</div>
            <div class="summary-value">
              {{ reportData.summary.totalActualPacks }}
            </div>
            <div class="summary-unit">packs</div>
          </div>
          <div class="summary-card">
            ...
            <table class="data-table">
              ...
              <tbody>
                <tr v-for="shift in reportData.shifts" :key="shift.shift">
                  ...
                  <td>{{ shift.targetPacks }}</td>
                  <td>{{ shift.actualPacks }}</td>
                  <td
                    :class="
                      getAchievementClass(shift.actualPacks, shift.targetPacks)
                    "
                  >
                    {{
                      ((shift.actualPacks / shift.targetPacks) * 100).toFixed(
                        1
                      )
                    }}%
                  </td>
                  <td class="reject-cell">{{ shift.rejectCount }}</td>
                  <td>{{ shift.avgWeight.toFixed(2) }}</td>
                  <td>{{ shift.minWeight.toFixed(2) }}</td>
                  <td>{{ shift.maxWeight.toFixed(2) }}</td>
                  <td
        ...
        <div class="breakdown-section">
          <h2 class="section-title">Weight Analysis by Shift</h2>
            ...
            >
              ...
              <div class="breakdown-metrics">
                <div class="breakdown-item">
                  <span class="breakdown-label">Average Weight:</span>
                  <span class="breakdown-value"
                    >{{ shift.avgWeight.toFixed(2) }} kg</span
                  >
                </div>
                <div class="breakdown-item">
                  <span class="breakdown-label">Min Weight:</span>
                  <span class="breakdown-value"
                    >{{ shift.minWeight.toFixed(2) }} kg</span
                  >
                </div>
                <div class="breakdown-item">
                  <span class="breakdown-label">Max Weight:</span>
                  <span class="breakdown-value"
                    >{{ shift.maxWeight.toFixed(2) }} kg</span
                  >
                </div>
                <div class="breakdown-item">
