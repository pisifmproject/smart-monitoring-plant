...
const lineId = "LINE_F_BAGMAKER";
const machineName = "Line F BagMaker";
// Date picker
...
async function loadShiftReports() {
    ...
    try {
        const response = await fetch(`http://localhost:2000/api/daily-report/bagmaker/${lineId}?date=${selectedDate.value}`);
        if (!response.ok) {
