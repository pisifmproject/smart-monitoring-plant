...
const generateDummyData = () => {
  gasData.value = {
    totalConsumption: 4500 + Math.random() * 2500,
    pressure: 2.5 + Math.random() * 1.5,
    flowRate: 65 + Math.random() * 35,
    temperature: 20 + Math.random() * 10,
    efficiency: 88 + Math.random() * 9,
    status: Math.random() > 0.85 ? "WARNING" : "NORMAL",
  };

  historicalData.value = Array.from({ length: 24 }, (_, i) => ({
    hour: `${String(i).padStart(2, "0")}:00`,
    consumption: 4000 + Math.random() * 3000,
  ...
  <div class="gas-dashboard">
        ...
        <div>
          <div class="flex items-center gap-3">
            <div class="p-2 bg-amber-500/10 rounded-lg">
              <Flame class="text-amber-400" :size="28" />
            </div>
            <div>
              ...
              <p class="text-sm text-slate-400">
                Plant {{ plantId }} - Real-time Gas Monitoring
              </p>
            </div>
          </div>
        </div>
    ...
    >
      ...
      <div>
        ...
        <p class="text-xs text-blue-400/70 mt-1">
          Real natural gas monitoring data coming soon. Currently showing
          simulated values.
        </p>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          ...
          <div>
            ...
            <div class="flex items-baseline gap-2">
              <span class="text-2xl font-bold text-white">{{
                formatNumber(gasData.totalConsumption)
              }}</span>
              <span class="text-xs text-slate-400">m³</span>
            </div>
          </div>
          <div class="p-2 bg-amber-500/10 rounded-lg">
            <Flame class="text-amber-400" :size="20" />
          </div>
        </div>
        <div class="flex items-center gap-1 text-xs">
          <TrendingUp class="text-emerald-400" :size="14" />
          <span class="text-emerald-400 font-semibold">+1.4%</span>
      ...
      >
        <div class="flex justify-between items-start mb-3">
          <div>
            ...
            <div class="flex items-baseline gap-2">
              <span class="text-2xl font-bold text-white">{{
                formatNumber(gasData.pressure, 2)
              }}</span>
        ...
        <p class="text-xs text-slate-500">Operating range: 2-4 bar</p>
      </div>
      ...
      >
        ...
        <p class="text-xs text-slate-500">Normal: 60-100 m³/h</p>
      </div>
      ...
      >
        <div class="flex justify-between items-start mb-3">
          <div>
            ...
            <div class="flex items-baseline gap-2">
              <span class="text-2xl font-bold text-white">{{
                formatNumber(gasData.efficiency, 1)
              }}</span>
    ...
    >
      ...
      <div class="overflow-x-auto">
        <div class="flex gap-2 min-w-max pb-4">
          ...
          >
            <div
              class="w-12 bg-amber-500/20 rounded-t"
              :style="{
                height: `${(data.consumption / 7000) * 200}px`,
                minHeight: '20px',
              }"
            ></div>
