...
// ---------------------------------------
// Extracted out of ProtectedLayout to prevent remounting/state loss on parent renders
const AppSidebar: React.FC<{ user: User; isCollapsed: boolean; onLogout: () => void }> = ({ user, isCollapsed, onLogout }) => {
    ...
    }, [location.pathname, plantContext]); // Add plantContext to dependency to ensure update
                                    ...
                                    <SidebarLink 
                                        ...
                                        onClick={(e) => {
                                            if (isActivePlant) {
                                                ...
                                                // When switching to a new plant, reset the states
        ...
        <div className="flex h-screen bg-slate-950 font-sans text-slate-200">
            <div className={`fixed inset-0 z-40 transform transition-transform duration-300 lg:hidden ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                <div className="w-72 bg-[#0b1120] border-r border-slate-800 flex flex-col h-full">
                    <AppSidebar user={user} isCollapsed={false} onLogout={onLogout} />
                </div>
            </div>
            ...
            <div className={`hidden lg:flex flex-col bg-[#0b1120] border-r border-slate-800 shrink-0 z-20 shadow-2xl transition-all duration-300 ${isSidebarOpen ? 'w-72' : 'w-20'}`}>
                <AppSidebar user={user} isCollapsed={!isSidebarOpen} onLogout={onLogout} />
            </div>
...
const PlantDashboardAccessWrapper = ({ user }: { user: User }) => {
    ...
    const hasAccess = user.role === UserRole.ADMINISTRATOR || (user.plantAccess?.includes(plantId as PlantCode));
    ...
    }, [hasAccess, user.plantAccess, navigate]);

    if (!hasAccess) return <SuspenseSpinner />;
...
const MachineDetailWrapper = React.memo(({ user }: { user: User }) => {
  ...
  const hasAccess = !machine || user.role === UserRole.ADMINISTRATOR || (user.plantAccess?.includes(machine.plantId));
  ...
  }, [machine, hasAccess, user.plantAccess, navigate]);
...
const LVMDPDetailWrapper = React.memo(({ user }: { user: User }) => {
  ...
  const hasAccess = !panel || user.role === UserRole.ADMINISTRATOR || (user.plantAccess?.includes(panel.plantId));
  ...
  }, [panel, hasAccess, user.plantAccess, navigate]);
...
const UtilitySummaryWrapper = React.memo(({ user }: { user: User }) => {
  ...
  const hasAccess = !plant || user.role === UserRole.ADMINISTRATOR || (user.plantAccess?.includes(plant.id));
  ...
  }, [plant, hasAccess, user.plantAccess, navigate]);
      ...
      <HashRouter>
        <Routes>
          ...
          >
            ...
            <Route path="dashboard/global" element={user ? <GlobalDashboard user={user} /> : null} />
            
            <Route path="plants/:plantId" element={user ? <PlantDashboardAccessWrapper user={user} /> : null} />
            <Route path="plants/:plantId/utilities" element={user ? <UtilitiesOverviewWrapper userRole={user.role} /> : null} />
            <Route path="plants/:plantId/production-lines" element={user ? <ProductionLinesOverviewWrapper userRole={user.role} /> : null} />
            
            <Route path="machines/:machineId" element={user ? <MachineDetailWrapper user={user} /> : null} />
            <Route path="lvmdp/:panelId" element={user ? <LVMDPDetailWrapper user={user} /> : null} />
            <Route path="utility/:type/:plantId" element={user ? <UtilitySummaryWrapper user={user} /> : null} />

            <Route path="settings" element={user ? <SettingsWrapper userRole={user.role} /> : null} />
