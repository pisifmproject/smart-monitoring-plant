        ...
        <div className="space-y-8 animate-in fade-in duration-300 w-full relative">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                    {userRole !== UserRole.OPERATOR && (
                         <button onClick={() => navigate('/app/dashboard/global')} className="p-2 hover:bg-slate-800 rounded-full transition-colors text-slate-300 hover:text-white">
                            <ArrowLeft size={24} />
                        </button>
                    )}
                    <div>
                        ...
                        <p className="text-slate-300 text-sm font-medium mt-0.5">{plant.location}</p>
                    </div>
                </div>
                         ...
                         {utilityMetrics.map((metric) => (
                            isDataItemVisible(userRole, metric.key, visibilityContext) && (
                                ...
                                >
                                    ...
                                    <div>
                                        <span className="text-xs text-slate-300 font-bold">{metric.label}</span>
                                            ...
                                            <tr key={shift.id} className={`transition-all duration-200 ${
                                                ...
                                                <td className="p-3 text-center">
                                                    <span className={`text-[10px] uppercase px-2 py-0.5 rounded-full font-bold ${
                                                        shift.status === 'ACTIVE' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-slate-700 text-slate-300'
                                                    }`}>
                                                        {shift.status}
                                                    </span>
                                                </td>
                                        ...
                                        >
                                            <div className="flex items-center gap-3 min-w-0">
                                                ...
                                                <div className="min-w-0">
                                                    ...
                                                    <p className="text-xs text-slate-300 truncate">{alarm.timestamp} • {alarm.source}</p>
                                                </div>
                                            </div>
            ...
            <div>
                <h3 className="text-xl font-bold text-white mb-4 flex items-center gap-2">
                    <Activity className="text-slate-300" /> Production Lines
                </h3>
                            ...
                            >
                                ...
                                <div className="grid grid-cols-2 gap-4 mt-auto pt-3 border-t border-slate-700/50">
                                    {isDataItemVisible(userRole, 'MACHINE_CARD_OUTPUT', machineContext) && (
                                        <div>
                                            <p className="text-slate-300 text-xs font-bold uppercase">Output</p>
                                    ...
                                    {isDataItemVisible(userRole, 'MACHINE_CARD_OEE', machineContext) && (
                                        <div>
                                            <p className="text-slate-300 text-xs font-bold uppercase">OEE</p>
