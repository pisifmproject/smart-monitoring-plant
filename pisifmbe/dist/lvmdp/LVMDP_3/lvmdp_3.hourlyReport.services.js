...
const generateHourlyReportsFromView = async (dateStr) => {
  ...
  `;
    ...
    const t1 = Date.now();
    console.log(`[LVMDP3 Hourly] DB aggregation: ${t1 - t0}ms, ${rows.length} hours`);
    ...
    if (hourlyData.length > 0) {
        const t2 = Date.now();
        await (0, lvmdp_3_hourlyReport_repository_1.batchSaveHourlyReports)(hourlyData);
        const t3 = Date.now();
        console.log(`[LVMDP3 Hourly] Saved ${hourlyData.length} records in ${t3 - t2}ms`);
    }
    const totalTime = Date.now() - t0;
    console.log(`[LVMDP3 Hourly] Total time: ${totalTime}ms`);
    return hourlyData;
};
...
const fetchHourlyReportByDate = async (dateStr) => {
    const result = await (0, lvmdp_3_hourlyReport_repository_1.getHourlyReportByDate)(dateStr);
...
const fetchHourlyReportByRange = async (startDate, endDate) => {
    return await (0, lvmdp_3_hourlyReport_repository_1.getHourlyReportByRange)(startDate, endDate);
};
...
const fetchHourlyReportByDateHour = async (dateStr, hour) => {
    return await (0, lvmdp_3_hourlyReport_repository_1.getHourlyReportByDateHour)(dateStr, hour);
};
