...
const lvmdp_4_dailyReport_services_1 = require("./lvmdp_4.dailyReport.services");
...
router.get("/all", async (req, res) => {
    try {
        ...
        const { getShiftAveragesLVMDP4 } = await Promise.resolve().then(() => __importStar(require("./lvmdp_4.services")));
...
router.get("/month", async (req, res) => {
    try {
        ...
        const reports = await (0, lvmdp_4_dailyReport_services_1.fetchMonthlyReport)(year, month);
        res.json({
            success: true,
            data: reports,
        });
...
router.get("/hourly/:date", async (req, res) => {
    try {
        ...
        const { getDailyReportByDate } = await Promise.resolve().then(() => __importStar(require("./lvmdp_4.dailyReport.repository")));
        const existing = await getDailyReportByDate(new Date(date));
        if (!existing || existing.length === 0) {
            try {
                await (0, lvmdp_4_dailyReport_services_1.generateAndSaveDailyReport)(date);
            }
            catch (genErr) {
                // lanjut, tidak perlu error
            }
        }
        const rows = await (0, lvmdp_4_dailyReport_services_1.fetchHourlyAggregates)(date);
...
router.get("/:date", async (req, res) => {
    try {
        ...
        const report = await (0, lvmdp_4_dailyReport_services_1.fetchDailyReport)(dateStr);
...
router.post("/generate", async (req, res) => {
    try {
        ...
        const saved = await (0, lvmdp_4_dailyReport_services_1.generateAndSaveDailyReport)(dateStr);
        res.status(201).json({
            success: true,
...
router.get("/hourly/:date", async (req, res) => {
    try {
        ...
        const { fetchHourlyReportByDate } = await Promise.resolve().then(() => __importStar(require("./lvmdp_4.hourlyReport.services")));
...
/**
 * POST /api/lvmdp4/daily-report/current-shift
 * Generate and save current shift report (real-time)
 */
router.post("/current-shift", async (req, res) => {
    try {
        const { saveCurrentShiftReport, getCurrentShift } = await Promise.resolve().then(() => __importStar(require("./lvmdp_4.dailyReport.services")));
        ...
        console.error("[LVMDP4 Daily Controller] Error saving current shift:", err);
        return res.status(500).json({
            success: false,
