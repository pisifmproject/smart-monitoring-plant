...
const lvmdp_4_dailyReport_repository_1 = require("./lvmdp_4.dailyReport.repository");
const lvmdp_4_services_1 = require("./lvmdp_4.services");
...
const saveShiftReport = async (dateStr, shiftNumber) => {
    ...
    // Get shift data
    const shifts = await (0, lvmdp_4_services_1.getShiftAveragesLVMDP4)(dateStr);
    const shiftData = shifts[`shift${shiftNumber}`];
    // Check if report exists
    const reportDate = makeUtcDateFromYmd(dateStr);
    const existing = await (0, lvmdp_4_dailyReport_repository_1.getDailyReportByDate)(reportDate);
    const now = new Date();
    if (existing.length > 0) {
        ...
        return await (0, lvmdp_4_dailyReport_repository_1.saveDailyReport)({
            ...existing[0],
            ...updateData,
        });
    }
        ...
        return await (0, lvmdp_4_dailyReport_repository_1.saveDailyReport)(reportData);
    }
};
...
const fetchDailyReport = async (dateStr) => {
    ...
    const reportDate = makeUtcDateFromYmd(dateStr);
    return await (0, lvmdp_4_dailyReport_repository_1.getDailyReportByDate)(reportDate);
};
...
const fetchMonthlyReport = async (year, month) => {
    ...
    return await (0, lvmdp_4_dailyReport_repository_1.getDailyReportByMonth)(year, month);
};
exports.fetchMonthlyReport = fetchMonthlyReport;
const fetchAllDailyReports = async () => {
    return await (0, lvmdp_4_dailyReport_repository_1.getAllDailyReports)();
};
...
const fetchHourlyAggregates = async (dateStr) => {
    ...
    return await (0, lvmdp_4_services_1.getHourlyAveragesLVMDP4)(dateStr);
};
