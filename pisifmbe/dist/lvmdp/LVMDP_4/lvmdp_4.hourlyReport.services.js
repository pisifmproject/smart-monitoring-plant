...
const generateHourlyReportsFromView = async (dateStr) => {
  ...
  `;
    ...
    const t1 = Date.now();
    console.log(`[LVMDP4 Hourly] DB aggregation: ${t1 - t0}ms, ${rows.length} hours`);
    ...
    if (hourlyData.length > 0) {
        const t2 = Date.now();
        await (0, lvmdp_4_hourlyReport_repository_1.batchSaveHourlyReports)(hourlyData);
        const t3 = Date.now();
        console.log(`[LVMDP4 Hourly] Saved ${hourlyData.length} records in ${t3 - t2}ms`);
    }
...
const fetchHourlyReportByRange = async (startDate, endDate) => {
    return await (0, lvmdp_4_hourlyReport_repository_1.getHourlyReportByRange)(startDate, endDate);
};
...
const fetchHourlyReportByDateHour = async (dateStr, hour) => {
    return await (0, lvmdp_4_hourlyReport_repository_1.getHourlyReportByDateHour)(dateStr, hour);
};
