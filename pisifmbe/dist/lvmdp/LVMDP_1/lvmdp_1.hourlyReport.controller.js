...
const lvmdp_1_hourlyReport_services_1 = require("./lvmdp_1.hourlyReport.services");
...
const getHourlyReport = async (req, res) => {
    try {
        ...
        const t0 = Date.now();
        const data = await (0, lvmdp_1_hourlyReport_services_1.fetchHourlyReportByDate)(date);
        const t1 = Date.now();
        return res.json({
            success: true,
            data,
...
const getHourlyReportRange = async (req, res) => {
    try {
        ...
        const t0 = Date.now();
        const data = await (0, lvmdp_1_hourlyReport_services_1.fetchHourlyReportByRange)(start, end);
        const t1 = Date.now();
        return res.json({
            success: true,
            data,
...
const generateHourlyReport = async (req, res) => {
    try {
        ...
        const data = await (0, lvmdp_1_hourlyReport_services_1.generateHourlyReportsFromView)(date);
        return res.json({
            success: true,
...
const generateSingleHour = async (req, res) => {
    try {
        ...
        const data = await (0, lvmdp_1_hourlyReport_services_1.generateSingleHourReport)(date, hour);
        if (!data) {
            return res.json({
                success: true,
        ...
        await (0, lvmdp_1_hourlyReport_repository_1.saveHourlyReport)(data);
        return res.json({
            success: true,
