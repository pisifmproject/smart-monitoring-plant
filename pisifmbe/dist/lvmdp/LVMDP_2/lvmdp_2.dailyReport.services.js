...
const saveShiftReport = async (dateStr, shiftNumber) => {
    ...
    const existing = await (0, lvmdp_2_dailyReport_repository_1.getDailyReportByDate)(reportDate);
    const now = new Date();
    if (existing.length > 0) {
        ...
        return await (0, lvmdp_2_dailyReport_repository_1.saveDailyReport)({
            ...existing[0],
            ...updateData,
        });
    }
...
const saveCurrentShiftReport = async () => {
    ...
    console.log(`[LVMDP2] Saving current shift ${shift} for ${date}`);
