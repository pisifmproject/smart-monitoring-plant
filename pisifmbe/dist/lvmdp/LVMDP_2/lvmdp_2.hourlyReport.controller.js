...
const lvmdp_2_hourlyReport_services_1 = require("./lvmdp_2.hourlyReport.services");
const lvmdp_2_hourlyReport_repository_1 = require("./lvmdp_2.hourlyReport.repository");
/**
 * GET /api/lvmdp/2/hourly?date=YYYY-MM-DD
...
const getHourlyReport = async (req, res) => {
    try {
        ...
        const t0 = Date.now();
        const data = await (0, lvmdp_2_hourlyReport_services_1.fetchHourlyReportByDate)(date);
        const t1 = Date.now();
        return res.json({
            success: true,
            data,
        ...
        console.error("[LVMDP2 Hourly Controller] Error:", err);
...
/**
 * GET /api/lvmdp/2/hourly/range?start=YYYY-MM-DD&end=YYYY-MM-DD
...
const getHourlyReportRange = async (req, res) => {
    try {
        ...
        const t0 = Date.now();
        const data = await (0, lvmdp_2_hourlyReport_services_1.fetchHourlyReportByRange)(start, end);
        const t1 = Date.now();
        return res.json({
            success: true,
            data,
...
/**
 * POST /api/lvmdp/2/hourly/generate
...
const generateHourlyReport = async (req, res) => {
    try {
        ...
        const data = await (0, lvmdp_2_hourlyReport_services_1.generateHourlyReportsFromView)(date);
        return res.json({
            success: true,
...
/**
 * POST /api/lvmdp/2/hourly/generate-hour
...
const generateSingleHour = async (req, res) => {
    try {
        ...
        const data = await (0, lvmdp_2_hourlyReport_services_1.generateSingleHourReport)(date, hour);
        if (!data) {
            return res.json({
                success: true,
        ...
        await (0, lvmdp_2_hourlyReport_repository_1.saveHourlyReport)(data);
        return res.json({
            success: true,
