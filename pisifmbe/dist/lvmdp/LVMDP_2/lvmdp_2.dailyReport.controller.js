...
const lvmdp_2_dailyReport_services_1 = require("./lvmdp_2.dailyReport.services");
...
router.get("/month", async (req, res) => {
    try {
        ...
        const reports = await (0, lvmdp_2_dailyReport_services_1.fetchMonthlyReport)(year, month);
        res.json({
            success: true,
            data: reports,
        });
...
router.get("/:date", async (req, res) => {
    try {
        ...
        const report = await (0, lvmdp_2_dailyReport_services_1.fetchDailyReport)(dateStr);
...
router.post("/generate", async (req, res) => {
    try {
        ...
        const saved = await (0, lvmdp_2_dailyReport_services_1.generateAndSaveDailyReport)(dateStr);
        res.status(201).json({
            success: true,
...
router.post("/current-shift", async (req, res) => {
    try {
        const { saveCurrentShiftReport, getCurrentShift } = await Promise.resolve().then(() => __importStar(require("./lvmdp_2.dailyReport.services")));
