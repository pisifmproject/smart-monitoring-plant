...
const mapHMIRow = (r: any): Lvmdp2Row => ({
  waktu:
...
export async function findLVMDPs(dateFrom?: string, dateTo?: string) {
  try {
    ...
    if (dateFrom && dateTo) {
      // Date range query
      query = sql`SELECT * FROM public.v_lvmdp_2 
...
let latestCache2: { data: any; timestamp: number } | null = null;
...
export async function findLatestLVMDP2() {
  ...
  try {
    const result = await db.execute(
      sql`SELECT * FROM public.v_lvmdp_2 
          ORDER BY waktu DESC 
          LIMIT 1`
    );
