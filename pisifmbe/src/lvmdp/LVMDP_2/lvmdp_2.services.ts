import { findLVMDPs } from "./lvmdp_2.repository";
 ...
 */
const getHourlyAveragesLVMDP2 = async (dateStr?: string) => {
  const today = dateStr ?? new Date().toISOString().slice(0, 10);
  const allRows = await findLVMDPs();
  ...
  const result = Array.from(hourlyMap.entries())
    ...
    .map(([hour, rows]) => {
      ...
      return {
        ...
        cos_phi: 0, // tidak ada data cos_phi di raw data shift calculation
        count: avg.count,
      };
    });
...
export { getAllLVMDPs, getShiftAveragesLVMDP2, getHourlyAveragesLVMDP2 };
