import { db } from "../../db";
import { dailyReportLVMDP4 } from "../../db/schema";
...
function makeDateRange(date: Date) {
  // return range as YYYY-MM-DD strings because reportDate column for LVMDP4
...
export const saveDailyReport = async (
  data: typeof dailyReportLVMDP4.$inferInsert
) => {
  return await db
    .insert(dailyReportLVMDP4)
    .values(data)
    .onConflictDoUpdate({
      target: dailyReportLVMDP4.reportDate,
      set: {
...
export const getDailyReportByDate = async (date: Date) => {
  ...
  return await db
    .select()
    .from(dailyReportLVMDP4)
    .where(
      and(
        gte(dailyReportLVMDP4.reportDate, start),
        lt(dailyReportLVMDP4.reportDate, end)
      )
    );
};
...
export const getDailyReportByMonth = async (year: number, month: number) => {
  ...
  return await db
    ...
    .orderBy(dailyReportLVMDP4.reportDate);
};
...
export const getAllDailyReports = async () => {
  return await db
    .select()
    .from(dailyReportLVMDP4)
    .orderBy(desc(dailyReportLVMDP4.reportDate));
};
...
export const getDailyReportsByDateRange = async (
  ...
  return await db
    ...
    .where(
      and(
        gte(dailyReportLVMDP4.reportDate, startDate),
        lt(dailyReportLVMDP4.reportDate, endDate)
      )
    )
    .orderBy(dailyReportLVMDP4.reportDate);
};
